...
class Mamba(nn.Module):
    ...
    def forward(self, x, batch_size):  # noqa
        """
        :param x: (B * L, C)
        ...
        y = SelectiveScan.apply(x, dts, As, Bs, Cs, Ds, delta_bias, True, 1)

        y = rearrange(y, 'b c (r l) -> b c r l', r=R)
        y = reduce(self.gamma * y, 'b c r l -> (b l) c', reduction='sum')
...
class HRSAMViT(BaseModule):
    ...
    def __init__(self,
        ...
        super(HRSAMViT, self).__init__(init_cfg)
        ...
        self.lateral_convs = nn.ModuleList([
            nn.Sequential(nn.Linear(embed_dim, final_embed_dim,
                                    bias=False),
    ...
    def forward(self, x):
        ...
        sizes = [(H, W)]  # compatible with multi-scale inputs
        for h, w in sizes:
            if (h, w) != (H, W):
                x = F.interpolate(
        ...
        for i, block in enumerate(self.blocks):
            if i in self.out_indices:
                x = self.mamba_blocks[out_index](x, batch_size=Bs)
                out_index += 1
            if i % 2 == 0:
            ...
            if self.use_checkpoint:
                ...
                x = block(x, **kwargs)
            if i in self.out_indices:
                outs = []  # compatible with multi-scale inputs
