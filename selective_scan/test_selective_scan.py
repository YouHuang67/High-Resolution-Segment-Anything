...
def build_selective_scan_fn(selective_scan_cuda: object = None,
                            mode="mamba_ssm", tag=None):
    MODE = mode

    class SelectiveScanFn(torch.autograd.Function):
        @staticmethod
        def forward(ctx, u, delta, A, B, C, D=None, z=None, delta_bias=None,
                    delta_softplus=False, return_last_state=False, nrows=1,
                    backnrows=-1):
            if u.stride(-1) != 1:
                u = u.contiguous()
            if delta.stride(-1) != 1:
            ...
            if not ctx.has_z:
                ...
                if MODE in ["mamba_ssm", "sstest"]:
                    out_z = rest[0]
                    return out_z if not return_last_state else (
                    out_z, last_state)
                elif MODE in ["sscore"]:
                    return out if not return_last_state else (out, last_state)
        ...
        def backward(ctx, dout, *args):
            ...
            if MODE in ["mamba_ssm"]:
                ...
                du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
                    ...
                    ctx.delta_softplus, ctx.backnrows
                )
            elif MODE in ["sscorendstate"]:
                du, ddelta, dA, dB, dC, dD, ddelta_bias, *rest = selective_scan_cuda.bwd(
                    u, delta, A, B, C, D, delta_bias, dout, x,
            ...
            if delta_bias is not None:
                if ddelta_bias.dtype != getattr(ctx, "_delta_bias_dtype",
                                                ddelta_bias.dtype):
    ...
    selective_scan_fn.__repr__ = lambda \
        *_: f"selective_scan_fn | {mode} | {tag}"
...
def selective_scan_ref_v2(u, delta, A, B, C, D=None, z=None, delta_bias=None,
                          delta_softplus=False,
                          return_last_state=False):
    """
    u: r(B D L)
    ...
    if A.is_complex():
        if is_variable_B:
            B = torch.view_as_complex(
                rearrange(B, "... (L two) -> ... L two", two=2))
        if is_variable_C:
            C = torch.view_as_complex(
                rearrange(C, "... (L two) -> ... L two", two=2))
    x = A.new_zeros((batch, dim, dstate))
    ys = []
    deltaA = torch.exp(torch.einsum('bdl,dn->bdln', delta, A))
    if not is_variable_B:
...
def selective_scan_fn(u, delta, A, B, C, D=None, z=None, delta_bias=None,
                      delta_softplus=False, return_last_state=False, *args,
                      **kwargs):
    return selective_scan_ref_v2(u, delta, A, B, C, D, z, delta_bias,
...
if MODE in ["mamba_ssm"]:
    ...
    selective_scan_fn = build_selective_scan_fn(selective_scan_cuda_core,
                                                mode=MODE)
    selective_scan_ref = selective_scan_ref
elif MODE in ["sstest"]:
    import selective_scan_cuda_test

    selective_scan_fn = build_selective_scan_fn(selective_scan_cuda_test,
                                                mode=MODE)
    selective_scan_ref = selective_scan_ref
    ...
    selective_scan_fn = build_selective_scan_fn(selective_scan_cuda_core,
                                                mode="sscore")
    selective_scan_ref = build_selective_scan_fn(selective_scan_cuda,
                                                 mode="mamba_ssm")
...
def test_selective_scan(is_variable_B, is_variable_C, varBC_groups, has_D,
                        has_z, has_delta_bias,
                        delta_softplus, return_last_state, seqlen, itype,
                        wtype, nrows, batch_size, dim, dstate):
    ...
    A = (-0.5 * torch.rand(dim, dstate, device=device,
                           dtype=wtype)).requires_grad_()
    if not is_variable_B:
        B_shape = (dim, dstate)
    elif varBC_groups == 1:
        B_shape = (
        batch_size, dstate, seqlen if not is_complex else seqlen * 2)
    else:
    ...
    if has_z:
        ...
        z = None
    if has_delta_bias:
        delta_bias = (0.5 * torch.rand(dim, device=device,
                                       dtype=torch.float32)).requires_grad_()
    else:
        delta_bias = None
    u = torch.randn(batch_size, dim, seqlen, device=device, dtype=itype,
                    requires_grad=True)
    delta = (0.5 * torch.rand(batch_size, dim, seqlen, device=device,
                              dtype=itype)).requires_grad_()
    ...
    assert torch.allclose(u.grad, u_ref.grad.to(dtype=itype), rtol=rtol * 2,
                          atol=atol * 2)
    assert torch.allclose(delta.grad, delta_ref.grad.to(dtype=itype),
                          rtol=rtol * 5, atol=atol * 10)
    ...
    assert torch.allclose(C.grad, C_ref.grad,
    ...
    if has_delta_bias:
        assert torch.allclose(delta_bias.grad, delta_bias_ref.grad, rtol=rtolw,
                              atol=atolw)
